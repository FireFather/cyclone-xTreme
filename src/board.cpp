...
int CastleSafe[ColourNb * 2 * 6];
int CastleEmpty[ColourNb * 2 * 6];
int KingSquare[ColourNb];
int RookSquare[ColourNb][2];
bool Chess960;
bool Arena;
...
bool board_is_ok( const board_t *board )
    ...
    if( board->piece_nb != board->piece_size[White] + board->pawn_size[White] + board->piece_size[Black]
...
void board_init_list( board_t *board )
    ...
    for ( colour = 0; colour < ColourNb; colour++ )
        ...
        if( board->number[WhiteBishop12] >= 10 )
        ...
        if( DEBUG )
            ...
            for ( i = 0; i < board->piece_size[colour]; i++ )
                ...
                if( i == 0 )
                    ...
                    {
                    ASSERT(!PIECE_IS_KING(board->square[sq]));
                    ASSERT(PIECE_ORDER(board->square[board->piece[colour][i]])
                        <= PIECE_ORDER(board->square[board->piece[colour][i - 1]]));
                    }
                }
            }
...
void frc_init()
    ...
    for ( int colour = 0; colour < ColourNb; colour++ )
        {
        kr[colour] = RookSquare[colour][CastleKing];
        qr[colour] = RookSquare[colour][CastleQueen];
        k[colour] = KingSquare[colour];
        }
    ...
    for ( sq = 0; sq < 24; sq++ )
        {
        CastleEmpty[sq] = SquareNone;
        CastleSafe[sq] = SquareNone;
        }

    for ( int colour = 0; colour < ColourNb; colour++ )
        {
        int B = FLIP(B1, colour);
        int C = FLIP(C1, colour);
        int D = FLIP(D1, colour);
        int F = FLIP(F1, colour);
        int G = FLIP(G1, colour);
        int j1, j2, j3, j4;
        j1 = j2 = j3 = j4 = 0;
        int king = KingSquare[colour];
        int qr = RookSquare[colour][CastleQueen];
        int kr = RookSquare[colour][CastleKing];
        ...
        for ( sq = B; sq <= G; sq++ )
            ...
            if( sq != king )
                ...
                if( qr != SquareNone )
                    ...
                    if( sq != qr )
                        {
                        if( (sq < king || sq == C && king == B) && sq >= C )
                            { // king
                            CASTLE_EMPTY(colour, CastleQueen)[j1++] = sq;
                            CASTLE_SAFE(colour, CastleQueen)[j2++] = sq;
                            }

                        else if( qr < sq && sq <= D || qr > sq && sq >= D ) // rook
                            CASTLE_EMPTY(colour, CastleQueen)[j1++] = sq;
                        }

                    else if( sq >= C ) // king through rook
                        CASTLE_SAFE(colour, CastleQueen)[j2++] = sq;
                    }
                ...
                if( kr != SquareNone )
                    ...
                    if( sq != kr )
                        ...
                        if( sq > king )
                            { // king
                            CASTLE_EMPTY(colour, CastleKing)[j3++] = sq;
                            CASTLE_SAFE(colour, CastleKing)[j4++] = sq;
                            }
                    ...
                    else if( sq <= G ) // king through rook
                        CASTLE_SAFE(colour, CastleKing)[j4++] = sq;
                    }
                }
            }
